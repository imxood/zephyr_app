...
static void write_kernro(void)
{
	...
	zassert_true(ptr < _image_rodata_end && ptr >= _image_rodata_start,
...
#elif defined(CONFIG_ARC)
K_APP_DMEM(part0)
s32_t size = (0 - CONFIG_PRIVILEGED_STACK_SIZE - STACK_GUARD_SIZE);
#endif
...
static void start_kernel_thread(void)
{
	...
	k_thread_create(&kthread_thread, kthread_stack, STACKSIZE,
			...
			K_PRIO_PREEMPT(1), K_INHERIT_PERMS, K_NO_WAIT);
...
static void read_other_stack(void)
{
	...
	k_thread_create(&uthread_thread, uthread_stack, STACKSIZE,
			(k_thread_entry_t)uthread_body, NULL, NULL, NULL, -1,
...
static void user_mode_enter(void)
{
	expect_fault = false;
	BARRIER();
	k_thread_user_mode_enter((k_thread_entry_t)umode_enter_func, NULL, NULL,
				 NULL);
}
...
static void access_other_memdomain(void)
{
	...
	k_thread_create(&uthread_thread, uthread_stack, STACKSIZE,
			(k_thread_entry_t)shared_mem_thread, NULL, NULL, NULL,
...
extern char Z_APP_START(access_part)[]; \
	extern char Z_APP_SIZE(access_part)[]; \
	struct k_mem_partition access_part = { \
		.start = (u32_t) &Z_APP_START(access_part), \
		.size = (u32_t) &Z_APP_SIZE(access_part), \
		.attr = K_MEM_PARTITION_P_RW_U_RW \
	}; \
	extern char Z_APP_BSS_START(access_part)[]; \
	extern char Z_APP_BSS_SIZE(access_part)[]; \
	Z_GENERIC_SECTION(.app_regions.name) \
	struct z_app_region access_part_region = { \
		.bss_start = &Z_APP_BSS_START(access_part), \
		.bss_size = (size_t) &Z_APP_BSS_SIZE(access_part) \
	}; \
	Z_APPMEM_PLACEHOLDER(access_part);
...
static void spawn_user(void)
{
	...
	k_thread_create(&kthread_thread, kthread_stack, STACKSIZE,
			...
			K_PRIO_PREEMPT(1), K_INHERIT_PERMS | K_USER, K_NO_WAIT);
...
void scenario_entry(void *stack_obj, size_t obj_size)
{
	...
	stest_thread_launch(stack_obj, obj_size, K_INHERIT_PERMS, true);
}
...
#endif
	k_thread_access_grant(k_current_get(), &kthread_thread, &kthread_stack,
			      &uthread_thread, &uthread_stack,
			      &uthread_start_sem, &uthread_end_sem,
			      &test_revoke_sem, &kpipe, &expect_fault_sem);
	ztest_test_suite(userspace, ztest_user_unit_test(is_usermode),
