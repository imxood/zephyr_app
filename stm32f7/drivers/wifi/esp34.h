...
#define LOG_MODULE_NAME wifi_esp32
...
K_THREAD_STACK_DEFINE(esp32_workq_stack,
		      1024);
...
static struct esp32_data foo_data;
			...
			struct net_context **context)
{
	...
	for (i = 0; i < ESP32_MAX_CONNECTIONS && foo_data.sock_map & BIT(i); i++){}
...
static int esp32_init(struct device *dev)
{
	...
	k_delayed_work_init(&foo_data.ip_addr_work, esp32_ip_addr_work);
	...
	if (mdm_receiver_register(&foo_data.mdm_ctx, CONFIG_WIFI_ESP32_UART_DEVICE,
...
NET_DEVICE_OFFLOAD_INIT(esp32, "ESP32", esp32_init, &foo_data, NULL, 80, &esp32_api, 1500);
